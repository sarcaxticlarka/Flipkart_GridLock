"""
Traffic Demand Prediction — v7 Optimized Pipeline (Target: stronger 94+)
...
6. Row-wise Day49-vs-Day48 context correction features for ratios, deltas, slope, and profile blends.
7. Minute-level target encodings and a validation-tuned stacking ensemble.
"""
...
te_gh_min  = smooth_te(train_48, ['geohash', 'minute_of_day'], k=8)
te_g5_min  = smooth_te(train_48, ['geo_5',   'minute_of_day'], k=15)
te_g4_hour = smooth_te(train_48, ['geo_4',   'hour'], k=25)
te_road_hr = smooth_te(train_48, ['RoadType', 'NumberofLanes', 'hour'], k=30)
...
train = train.merge(te_gh_min,  on=['geohash', 'minute_of_day'], how='left')
train = train.merge(te_g5_min,  on=['geo_5',   'minute_of_day'], how='left')
train = train.merge(te_g4_hour, on=['geo_4',   'hour'], how='left')
train = train.merge(te_road_hr, on=['RoadType', 'NumberofLanes', 'hour'], how='left')
...
test  = test.merge(te_gh_min,  on=['geohash', 'minute_of_day'], how='left')
test  = test.merge(te_g5_min,  on=['geo_5',   'minute_of_day'], how='left')
test  = test.merge(te_g4_hour, on=['geo_4',   'hour'], how='left')
test  = test.merge(te_road_hr, on=['RoadType', 'NumberofLanes', 'hour'], how='left')

for col in [
    'te_geohash_hour', 'te_geo_5_hour', 'te_geo_3_is_morning_peak', 'te_geohash',
    'te_geohash_minute_of_day', 'te_geo_5_minute_of_day', 'te_geo_4_hour',
    'te_RoadType_NumberofLanes_hour'
]:
...
train['te_geohash_minute_scaled'] = train['te_geohash_minute_of_day'] * train['d49_d48_ratio']
test['te_geohash_minute_scaled']  = test['te_geohash_minute_of_day'] * test['d49_d48_ratio']
train['te_geo_5_minute_scaled'] = train['te_geo_5_minute_of_day'] * train['d49_d48_ratio']
test['te_geo_5_minute_scaled']  = test['te_geo_5_minute_of_day'] * test['d49_d48_ratio']
...
# ─────────────────────────────────────────────────────────────────────────────
# ROW-WISE DAY49-vs-DAY48 CONTEXT CORRECTION FEATURES
# ─────────────────────────────────────────────────────────────────────────────

CONTEXT_SLOTS = [0, 15, 30, 45, 60, 75, 90, 105, 120]

def add_context_correction_features(df, is_train=True):
    out = df.copy()
    n = len(out)

    ratio_mat = np.full((n, len(CONTEXT_SLOTS)), np.nan, dtype=float)
    delta_mat = np.full((n, len(CONTEXT_SLOTS)), np.nan, dtype=float)

    for j, slot in enumerate(CONTEXT_SLOTS):
        same_col = f'same_day_lag_{slot}'
        if same_col not in out.columns:
            continue

        d48_slot = cascade_lookup(
            [pivot_48, pivot_48_g5, pivot_48_g4, pivot_48_g3, pivot_48_g2],
            [out['geohash'].values, out['geo_5'].values, out['geo_4'].values,
             out['geo_3'].values, out['geo_2'].values],
            np.full(n, slot, dtype=int)
        )
        gb_slot = float(pivot_48_gb[slot].iloc[0]) if slot in pivot_48_gb.columns else GLOBAL_MEAN
        d48_slot = np.where(np.isnan(d48_slot), gb_slot, d48_slot)

        same_vals = out[same_col].astype(float).values
        valid = np.isfinite(same_vals) & np.isfinite(d48_slot) & (d48_slot > 1e-6)
        ratio_mat[valid, j] = np.clip(same_vals[valid] / d48_slot[valid], 0.25, 4.0)
        delta_mat[valid, j] = same_vals[valid] - d48_slot[valid]

    weights = np.arange(1, len(CONTEXT_SLOTS) + 1, dtype=float)
    valid_ratio = np.isfinite(ratio_mat)
    valid_delta = np.isfinite(delta_mat)

    ratio_fill = np.where(valid_ratio, ratio_mat, np.nan)
    delta_fill = np.where(valid_delta, delta_mat, np.nan)

    out['ctx_ratio_mean'] = np.nanmean(ratio_fill, axis=1)
    out['ctx_ratio_median'] = np.nanmedian(ratio_fill, axis=1)
    out['ctx_ratio_std'] = np.nanstd(ratio_fill, axis=1)
    out['ctx_ratio_min'] = np.nanmin(ratio_fill, axis=1)
    out['ctx_ratio_max'] = np.nanmax(ratio_fill, axis=1)

    weighted_ratio_num = np.nansum(np.where(valid_ratio, ratio_mat * weights, 0.0), axis=1)
    weighted_ratio_den = np.sum(np.where(valid_ratio, weights, 0.0), axis=1)
    out['ctx_ratio_wmean'] = weighted_ratio_num / np.where(weighted_ratio_den == 0, np.nan, weighted_ratio_den)

    weighted_delta_num = np.nansum(np.where(valid_delta, delta_mat * weights, 0.0), axis=1)
    weighted_delta_den = np.sum(np.where(valid_delta, weights, 0.0), axis=1)
    out['ctx_delta_mean'] = np.nanmean(delta_fill, axis=1)
    out['ctx_delta_wmean'] = weighted_delta_num / np.where(weighted_delta_den == 0, np.nan, weighted_delta_den)

    out['ctx_count'] = valid_ratio.sum(axis=1)
    out['ctx_last_ratio'] = np.nan
    out['ctx_last_delta'] = np.nan
    out['ctx_ratio_slope'] = np.nan

    slot_arr = np.array(CONTEXT_SLOTS, dtype=float)
    for i in range(n):
        idx = np.flatnonzero(valid_ratio[i])
        if len(idx) == 0:
            continue
        out.iat[i, out.columns.get_loc('ctx_last_ratio')] = ratio_mat[i, idx[-1]]
        out.iat[i, out.columns.get_loc('ctx_last_delta')] = delta_mat[i, idx[-1]]
        if len(idx) >= 2:
            out.iat[i, out.columns.get_loc('ctx_ratio_slope')] = np.polyfit(slot_arr[idx], ratio_mat[i, idx], 1)[0]

    for col in [
        'ctx_ratio_mean', 'ctx_ratio_median', 'ctx_ratio_wmean', 'ctx_last_ratio',
        'ctx_ratio_min', 'ctx_ratio_max'
    ]:
        out[col] = out[col].replace([np.inf, -np.inf], np.nan).fillna(out['d49_d48_ratio'])

    for col in ['ctx_ratio_std', 'ctx_delta_mean', 'ctx_delta_wmean', 'ctx_last_delta', 'ctx_ratio_slope']:
        out[col] = out[col].replace([np.inf, -np.inf], np.nan).fillna(0.0)

    base = out['d48_same_slot'].fillna(out['te_geohash_hour']).fillna(GLOBAL_MEAN)
    out['profile_ctx_mean'] = base * out['ctx_ratio_mean']
    out['profile_ctx_wmean'] = base * out['ctx_ratio_wmean']
    out['profile_ctx_last'] = base * out['ctx_last_ratio']
    out['profile_ctx_delta'] = base + out['ctx_delta_wmean']

    elapsed_from_context = np.maximum(out['minute_of_day'].astype(float) - 120.0, 0.0)
    out['profile_ctx_trend'] = base * np.clip(
        out['ctx_last_ratio'] + out['ctx_ratio_slope'] * elapsed_from_context,
        0.25,
        4.0
    )

    for col in ['profile_ctx_mean', 'profile_ctx_wmean', 'profile_ctx_last', 'profile_ctx_delta', 'profile_ctx_trend']:
        out[col] = out[col].clip(0.0, 1.0)

    return out

print('Adding Day49/Day48 context correction features...')
train = add_context_correction_features(train, is_train=True)
test  = add_context_correction_features(test,  is_train=False)
...
# The correction/profile columns are powerful direct predictors, but feeding all
# of them into every tree made the local holdout worse. Keep the base learners
# close to the v6 feature set and expose these signals to the final blender.
META_ONLY_COLS = [
    'ctx_ratio_mean', 'ctx_ratio_median', 'ctx_ratio_std', 'ctx_ratio_min', 'ctx_ratio_max',
    'ctx_ratio_wmean', 'ctx_delta_mean', 'ctx_delta_wmean', 'ctx_count', 'ctx_last_ratio',
    'ctx_last_delta', 'ctx_ratio_slope', 'profile_ctx_mean', 'profile_ctx_wmean',
    'profile_ctx_last', 'profile_ctx_delta', 'profile_ctx_trend',
    'te_geohash_minute_of_day', 'te_geo_5_minute_of_day', 'te_geo_4_hour',
    'te_RoadType_NumberofLanes_hour', 'te_geohash_minute_scaled', 'te_geo_5_minute_scaled',
]
FEATURE_COLS = [c for c in FEATURE_COLS if c not in META_ONLY_COLS]
...
# Fit Meta Learner
META_BLEND_COLS = [
    'synthetic_pred', 'd48_same_slot', 'd48_off_0_scaled', 'te_geohash_hour_scaled',
    'te_geohash_scaled', 'profile_ctx_mean', 'profile_ctx_wmean',
    'profile_ctx_last', 'profile_ctx_delta', 'profile_ctx_trend'
]
meta_val_parts = [p_lgbm_val, p_xgb_val, p_hgb_val]
for col in META_BLEND_COLS:
    if col in train.columns:
        vals = train.loc[valid_mask, col].astype(float).fillna(GLOBAL_MEAN).values
        meta_val_parts.append(np.clip(vals, 0.0, 1.0))
meta_X = np.column_stack(meta_val_parts)
...
meta_test_parts = [p_lgbm_test, p_xgb_test, p_hgb_test]
for col in META_BLEND_COLS:
    if col in test.columns:
        vals = test[col].astype(float).fillna(GLOBAL_MEAN).values
        meta_test_parts.append(np.clip(vals, 0.0, 1.0))
meta_test_X = np.column_stack(meta_test_parts)
